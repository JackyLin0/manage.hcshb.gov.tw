...
<script>
  function add()
  {
    document.mform.action="email101_add.jsp";
  ...
  function mdy(serno)
  {
     ...
     document.mform.action="email101_mdy.jsp";
     document.mform.method="post";
     document.mform.submit();
  } 
  ...
  function del()
  {
     ...
     if(x)
     {
        document.mform.action="email101_del.jsp?type=0";
        document.mform.method="post";
        document.mform.submit();
     } 
  } 
...
<%
  ...
  VirusEmailData query = new VirusEmailData();    
...
<div>
  ...
  <span class="T11b">E-Mail</span>
  <input name="keyword" type="text" class="lInput01" size="15" value="<%=(keyword.equals("") ? "請輸入Email" : keyword)%>" onFocus="javascript: if (this.value=='請輸入Email') this.value='';" onBlur="javascript: if(this.value=='') this.value='請輸入Email';">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
  ...
  <%
    ...
    for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	VirusEmailData qlist = ( VirusEmailData )qlists.get( i ); %>
    	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	  ...
    	  <td align="left"><a href="javascript:mdy('<%=qlist.getSerno()%>')"><%=qlist.getPubunitname()%></a></td>
    	  <td align="left"><%=qlist.getLiaisonemail()%></td>
    	  <%
    	    String updatedate = "";
