/*
 * 撰寫日期：2008/10/26
 * 程式名稱：OnLineApplicationData.java
 * 功能：
 * 撰寫者：張資梅
 */
...
public class OnLineApplicationData extends WebSiteLog {
    ...
    public boolean create()
    {
    	...
    	try
    	{
    		...
    		String sql =  "insert into " + tablename;
    		       sql += "(Serno,PubUnitDN,PubUnitName,Mserno,Mclassname,Subject,Content,CloseDate,StartUsing,RelateUrl,RelateName,Fsort,PostName,CreateDate,UpdateDate)";
    		       sql += " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public ArrayList<Object> findByday( String tablename, String keyword, String punit, String qclass )
    {
    	...
    	try
    	{
    		...
    		{
    			OnLineApplicationData tmpQuery = new OnLineApplicationData();
    ...
    public boolean store()
    {
    	...
    	try
    	{
    		...
    		stmts2 = conn.prepareStatement(
    			"update " + tablename + " set Mserno = ?, Mclassname = ?, Subject = ?, Content = ?, CloseDate = ?, StartUsing = ?, RelateUrl = ?, RelateName = ?, Fsort = ?, PostName = ?, UpdateDate = ? where Serno = ? and PubUnitDN = ?");
    		...
    		stmts2.setString(12, serno );
    		stmts2.setString(13, pubunitdn );
