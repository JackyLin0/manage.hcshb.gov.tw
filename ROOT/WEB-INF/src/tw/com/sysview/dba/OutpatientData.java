...
public class OutpatientData extends WebSiteLog {
    ...
    public boolean create()
    {
    	...
    	try
    	{
    		...
    		String sql =  "insert into " + tablename;
    		       sql += "(Serno,PubUnitDN,PubUnitName,Subject,Secsubject,Content,PosterDate,CloseDate,StartUsing,RelateUrl,RelateName,Fsort,PostName,CreateDate,UpdateDate)";
    		       sql += " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public ArrayList<Object> findByday( String tablename, String keyword, String punit )
    {
    	...
    	/*	if ( qclass != null && !qclass.equals("") ) {
    		...
    		{
    			OutpatientData tmpQuery = new OutpatientData();
    ...
    public boolean store()
    {
    	...
    	try
    	{
    		...
    		stmts2 = conn.prepareStatement(
    			"update " + tablename + " set Subject = ?, Secsubject = ?, Content = ?, PosterDate = ?, CloseDate = ?, StartUsing = ?, RelateUrl = ?, RelateName = ?, Fsort = ?, PostName = ?, UpdateDate = ? where Serno = ? and PubUnitDN = ?");
    		...
    		if ( updateRow2 < 0 )
    		{
    			try {
    				conn.rollback();
    				errorMsg = "Insert into table fail(Outpatient).";
    				System.out.println(errorMsg);
    				return false;
 	            }catch(Exception backerr){
 	            	System.out.println("rollback faild!");
