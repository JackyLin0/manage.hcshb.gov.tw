...
public class BigBannerData extends WebSiteLog {
    ...
    public ArrayList<Object> findByday( String tablename, String keyword, String websitedn, String sdate1, String sdate2 )
    {
    	...
    	try
    	{
    		...
    		{
    			BigBannerData tmpQuery = new BigBannerData();
    	...
    	try
    	{
            //同意易動設定
     		...
     		stmts = conn.prepareStatement(
	    			"update " + tablename + " set Subject = ?, PosterDate = ?, CloseDate = ?, StartUsing = ?, RelateUrl = ?, RelateName = ?, ClientFile = ?, ServerFile = ?, ExpFile = ?, ImageMagick = ?, Fsort = ?, PostName = ?, UpdateDate = ? where Serno = ? and PubUnitDN = ?");
	    	
     		stmts.setString(1, subject );
     		stmts.setString(2, posterdate );
     		stmts.setString(3, closedate );
     		stmts.setString(4, startusing );
     		stmts.setString(5, relateurl );
     		stmts.setString(6, relatename );
     		stmts.setString(7, clientfile );
     		stmts.setString(8, serverfile );
     		stmts.setString(9, expfile );
     		stmts.setString(10, imagemagick );
     		stmts.setInt(11, fsort );
     		stmts.setString(12, postname );
     		stmts.setString(13, getNowYear() );
     		stmts.setString(14, serno );
     		stmts.setString(15, pubunitdn );
