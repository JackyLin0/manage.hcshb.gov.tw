...
public class TopicClassData extends WebSiteLog {
    ...
    public boolean create()
    {
    	...
    	try
    	{
    		...
    		//找最大序號 start
    		stmts = conn.prepareStatement("select max(serno) as seqno from " + tablename + " where substring(serno,1,8) = ? and WebSiteDN = '" + websitedn + "'" );
    		...
    		String sql =  "insert into " + tablename;
    		       sql += "(Serno,WebSiteDN,WebSiteName,PubUnitDN,PubUnitName,ClassName,PostName,CreateDate,UpdateDate)";
    		       sql += " values(?,?,?,?,?,?,?,?,?)";
    		...
    		stmts.setString(4, pubunitdn );
    		stmts.setString(5, pubunitname );
	...
	//查詢多筆資料及筆數
    public ArrayList<Object> findByday( String tablename, String punit, String classname, String websitedn )
    {
    	Connection conn = null;
    	try
    	{
    	...
    	try
    	{
    		...
    		{
    			TopicClassData tmpQuery = new TopicClassData();
    ...
    public boolean load( String serno, String tablename )
    {
    	...
    	try
    	{
    		...
    		stmts.clearParameters();
    		System.out.println("select * from " + tablename + " where Serno = '" + serno + "' and WebSiteDN = '" + websitedn + "'");
    		rs = stmts.executeQuery();
    		if ( !rs.next() )
    		{
    ...
    public boolean store()
    {
    	...
    	try
    	{
    		...
    		stmts = conn.prepareStatement(
    				"update " + tablename + " set ClassName = ?, UpdateDate = ? where Serno = ? and WebSiteDN = ?" ); 
    		stmts.clearParameters();    	 	
    ...
    public ArrayList<Object> findByday( String tablename, String websitedn )
    {
    	...
    	try
    	{
    		...
    		sSql.append( "select * from " + tablename + " where WebSiteDN = '" + websitedn + "' order by PubUnitDN,Serno" );
