...
public class WebUnitData extends WebSiteLog {
    ...
    public boolean create()
    {
    	...
    	try
    	{
    		...
    		String sql =  "insert into " + tablename;
    		       sql += "(Serno,PubUnitDN,PubUnitName,Mserno,Mclassname,Subject,StartUsing,RelateUrl,RelateName,Fsort,PostName,CreateDate,UpdateDate)";
    		       sql += " values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	//查詢多筆資料及筆數
    public ArrayList<Object> findByday( String tablename, String keyword, String punit, String classname )
    {
    	Connection conn = null;
    	try
    	{
    	...
    	try
    	{
    		...
    		if ( classname != null && !classname.equals("") ) {
    			String[] ary_class = SvString.split(classname,"-");
    			sSql.append( " and Mserno = '" + ary_class[0] + "'" );
    		}
    		
    		sSql.append( " order by Serno,UpdateDate desc,PubUnitDN" );
    		...
    		{
    			WebUnitData tmpQuery = new WebUnitData();
    ...
    public boolean store()
    {
    	...
    	try
    	{
    		...
    		stmts2 = conn.prepareStatement(
    			"update " + tablename + " set Mserno = ?, Mclassname = ?, Subject = ?, StartUsing = ?, RelateUrl = ?, RelateName = ? , Fsort = ?, PostName = ?, UpdateDate = ? where Serno = ? and PubUnitDN = ?");
