...
public class SchoolData extends WebSiteLog {
    ...
    public boolean create()
    {
    	...
    	try
    	{
            //同意易動設定
     		...
     		//查詢帳號是否存在
     		stmts3 = conn.prepareStatement( "select * from SchoolData where Uid = '" + uid + "'");
    		stmts3.clearParameters();
    		
    		rs3 = stmts3.executeQuery();
    		if ( rs3.next() )
    		{
    			errorMsg = "此帳號：【" + uid + "】已存在，請重新輸入！";
    			return false;
    		}
    		...
    		String sql =  "insert into " + tablename;
    		       sql += "(Serno,PubUnitDN,PubUnitName,SchoolName,Uid,PassWord,LiaisonPer,LiaisonTel,CreateDate,PostName,UpdateDate)";
    		       sql += " values(?,?,?,?,?,?,?,?,?,?,?)";
    		...
    		stmts2.setString(4, schoolname );
    		stmts2.setString(5, uid );
    		stmts2.setString(6, uid );
    		stmts2.setString(7, liaisonper );
    		stmts2.setString(8, liaisontel );
    		stmts2.setString(9, getNowYear() );
    		stmts2.setString(10, postname );
    ...
    public ArrayList<Object> findByday( String tablename, String keyword, String punit )
    {
    	...
    	try
    	{
    		...
    		if ( keyword != null && !keyword.equals("") )  {
    			sSql.append( " and SchoolName like '%" + keyword + "%'" );
    		}
    		...
    		{
    			SchoolData tmpQuery = new SchoolData();
    			...
    			tmpQuery.setSchoolname( rs.getString( "SchoolName" ) );
    ...
    public boolean store()
    {
    	...
    	try
    	{
            //同意易動設定
     		...
     		//查詢帳號是否存在
     		stmts3 = conn.prepareStatement( "select * from SchoolData where Serno <> '" + serno + "' & Uid = '" + uid + "'");
    		stmts3.clearParameters();
    		...
    		stmts2 = conn.prepareStatement(
    			"update " + tablename + " set PubUnitDN = ?, PubUnitName = ?, SchoolName = ?, Uid = ?, LiaisonPer = ?, LiaisonTel = ?, PostName = ?, UpdateDate = ? where Serno = ?");
    		   		
    		stmts2.setString(1, pubunitdn );
    		stmts2.setString(2, pubunitname );
    		stmts2.setString(3, schoolname );
    		stmts2.setString(4, uid );
    		stmts2.setString(5, liaisonper );
    		stmts2.setString(6, liaisontel );
            ...
            //同意易動設定
     		...
     		for ( int i=0; i<ary_mserno.length; i++ ) {
        		...
        		if ( rs.next() ) {
        			...
        			this.setSubject(rs.getString( "SchoolName" ));
        			//網站維運統計共用參數(WebSiteLog)        			
            		if ( !createlog(conn,language) )
            		{
            			try {
    ...
    public boolean pwdreturn()
    {
    	...
    	try
    	{
    		...
    		//update
    		stmts2 = conn.prepareStatement(
    			"update SchoolData set PassWord = ? where Uid = ?");
    		   		
    		stmts2.setString(1, uid );
    		stmts2.setString(2, uid );
