...
<%
  ...
  <%}else{
	  ...
	  for ( int i=0; i<ary_delno.length; i++ ) {
		  MedResourcesData qfile = new MedResourcesData();
		  ArrayList<Object> qdels = qfile.findBydel("MedResourcesFile",ary_delno[i]);	  
		  int delrcount = qfile.getAllrecordCount();
		  if ( qdels != null ) {
			  for ( int d=0; d<delrcount; d++ ) {
				  MedResourcesData qdel = ( MedResourcesData )qdels.get( d );
	  ...
	  //執行動作(修改資料)  
	  rtn = obj.remove("MedResourcesFile");
	...
	<form name="mform" action="medical101.jsp" method="post">
