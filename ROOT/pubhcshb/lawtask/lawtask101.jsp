...
<script>
  function add()
  {
    document.mform.action="lawtask101_add.jsp";
  ...
  function qry()
  {
    document.mform.action="lawtask101.jsp";
    document.mform.method="post";
    document.mform.submit();
  }  
  ...
  function mdy(serno)
  {
     ...
     document.mform.action="lawtask101_mdy.jsp";
     document.mform.method="post";
     document.mform.submit();
  } 
  ...
  function del()
  {
     ...
     if(x)
     {
        document.mform.action="lawtask101_del.jsp?type=0";
        document.mform.method="post";
        document.mform.submit();
     } 
  } 
...
<%
  ...
  LawTaskData query = new LawTaskData();    
  ArrayList qlists = query.findByday(table2,keyword,qpunit,sdate1,sdate2,"LawTaskPoster");
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
  ...
  <%
    ...
    for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	LawTaskData qlist = ( LawTaskData )qlists.get( i ); %>
    	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	  ...
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','doc','LawTaskFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/file.gif" alt="file" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic','LawTaskFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/Cmd_Save.gif" alt="attachments" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <!-- <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','media','LawTaskFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/media1.gif" alt="media" width="16" height="16" border="0" align="absmiddle" /></a></td> -->
    	</tr> 
    <%}%>
