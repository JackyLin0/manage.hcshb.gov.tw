...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  LawTaskData qfile = new LawTaskData();
	  ArrayList qdels = qfile.findBydel("LawTaskFile",ary_delno[i]);	  	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  LawTaskData qdel = ( LawTaskData )qdels.get( d );
  ...
  LawTaskData obj = new LawTaskData();
...
<form name="mform" action="lawtask101.jsp" method="post">
