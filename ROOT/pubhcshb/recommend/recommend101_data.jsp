...
<script>
	...
	var FirstURL = "recommend101_mdy.jsp?islevel=1&websitedn=<%=websitedn%>&language=<%=language%>&t=<%=table%>"		 <!---Top Level URL--->
	var CloseImg1 = "<img src=image/-11.gif border=0>"	    <!---┼號圖檔--->
	var CloseImg2 = "<img src=image/-21.gif border=0>"	    <!---結尾┼號圖檔--->
	var OpenImg1 = "<img src=image/+11.gif border=0>"	    <!---─號圖檔--->
	var OpenImg2 = "<img src=image/+22.gif border=0>"	    <!---結尾─號圖檔--->
	var LineImg1 = "<img src=image/11.gif border=0>"	    <!---連接線圖檔--->
	var LineImg2 = "<br>"		                            <!---連接線圖檔--->
	var secTimeout = 60000		                            <!---資料讀取Timeout時間(毫秒)--->

	var ShowWaiting = "<body topmargin=0 leftmargin=0>"		<!---等待訊息--->
	var strFirstLine = "<tr><td colspan=50><img src='image/folder.gif'>&nbsp;<font fontface='細明體' size=3><b><%=websitename%></b></font></td></tr>"
	var objOpenWin = top
	<!---系統變數--->
	...
	var mIntervalID
	var mTimeoutID
	var strFirstHTML = "<form name='main' method='post'><table bolder=0 width='50%' align='center' cellPadding=0 cellSpacing=0>"

	<!---讀取資料--->
	ReadData("","",0,FirstURL)

function ReadData(strLineImg,strTrack,NowPoint,RunURL) {
	if (DataWin) if (!DataWin.closed) return		<!---避免重複點選之錯誤--->
	if (strTrack != "") {
		<!---切換為已開啟狀態--->
	...
	<!---顯示等待訊息--->
	if (ShowWaiting != "") {
		DataWin = objOpenWin.open("about:blank","TreeDataWin","left=" + ((screen.width/2) - 100) + ",top=" + ((screen.height/2) - 25) + ",width=200,height=100")
		DataWin.document.write(ShowWaiting)
	}
	<!---讀取資料，設定Timer抓取資料並設定Timeout時間--->
	DataWin = objOpenWin.open(RunURL,"TreeDataWin")
...
function GetTree() {
	...
	<!---判斷網頁資料是否已全部傳回--->
	try {
		if (!DataWin.TreeData) return
		if (DataWin.TreeData.innerHTML.indexOf("<!EndTreeData>") != -1) {
...
function ParseData() {
	...
	var strHTML = "<center>"
	var strCurHTML = DataWin.TreeData.innerHTML
