...
<script>
   function add()
   {
      document.mform.action="downfile101_add.jsp";
   ...
   function mdy(serno)
   {
      ...
      document.mform.action="downfile101_mdy.jsp";
      document.mform.method="post";
      document.mform.submit();
   } 
   ...
   function del()
   {
      ...
      if(x)
      {
         document.mform.action="downfile101_del.jsp?type=0";
         document.mform.method="post";
         document.mform.submit();
      } 
   }
...
<%
  ...
  DownFileData query = new DownFileData();  
  ArrayList<Object> qlists = query.findByday(table2,keyword,qpunit,sdate1,sdate2,"DownPoster");  
  ...
  <!-- 應用系統名稱 -->
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
     <tr align="center">
       <th width="4%">&nbsp;</th>
       <th width="4%">&nbsp;</th>
       <th width="34%" align="left"><%=lang.getSubject()%></th>
       <th width="9%"><%=lang.getShow()%></th>
       <th width="20%" align="left"><%=lang.getPubunitname()%></th>
       <th width="11%"><%=lang.getUpdatedate()%></th>
       <th width="9%"><%=lang.getDocfile()%></th>
       <th width="9%"><%=lang.getImgfile()%></th>
     ...
     <%
       ...
       for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	   DownFileData qlist = ( DownFileData )qlists.get( i ); %>
    	   <tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	     ...
    	     <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','doc','DownFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/file.gif" alt="file" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	     <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic','DownFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/Cmd_Save.gif" alt="attachments" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	     <!-- <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','media','DownFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/media1.gif" alt="media" width="16" height="16" border="0" align="absmiddle" /></a></td> -->
    	   </tr> 
       <%}%>
