...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  ...
	  ArrayList qdels = qfile.findBydel("DownFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  DownFileData qdel = ( DownFileData )qdels.get( d );
...
<form name="mform" action="downfile101.jsp" method="post">
