...
<script>
  ...
  function back()
  {
     document.mform.action="receive.jsp";
  ...
  {
     document.mform.action = "receive_addsave.jsp?rcount=" + rcount;
     document.mform.method="post";
     document.mform.submit();
  }    
</script>
...
<%
  ...
  ArrayList<Object> qwebs = qwebsite.findByday(mlanguage,"","topicsite");
  int webrcount = qwebsite.getAllrecordCount();
  
  //尋找已設定的發布站台
  PublishData query = new PublishData();  
  ArrayList<Object> qownwebs = query.findByday(table,aplistdn,"R");
...
<p></p>
 <a class="md" href="javascript:save('<%=webrcount%>')"><%=lang.getSave()%></a>	
    	...
    	for ( int w=0; w<webrcount; w++ ) {
    		...
    		<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    		  ...
    		  <%
    		    ...
    		    String isSelected = "";
    		    String[] ary_ownwebsite1 = SvString.split(ownwebsite,"||");
    		    if ( SvString.contain(ary_ownwebsite1,qweb.getWebsitedn()) )
    		    	isSelected = "checked";
    		  %>
    		  <td><input type="checkbox" name="check<%=w%>" value="<%=datavalue%>" <%=isSelected%> /></td>
    		  <td><%=qweb.getWebsitename()%></td>
    		  <%
    		    PublishData qisexam = new PublishData();
    		    boolean rtn = qisexam.load("PublishData",aplistdn,website,qweb.getWebsitedn(),"R");    		    
    		  %>
    		  <td><span class="T12b">審核</span>&nbsp;&nbsp;
    		    <%
    		      if ( rtn ) { %>
    		    	  <input type="radio" name="isexamine<%=w%>" value="Y" <%=((qisexam.getIsexamine().equals("Y")) ? "checked" : "")%> />是&nbsp;&nbsp;
    		    	  <input type="radio" name="isexamine<%=w%>" value="N" <%=((qisexam.getIsexamine().equals("N")) ? "checked" : "")%> />否
    		      <%}else{%>
    		    	  <input type="radio" name="isexamine<%=w%>" value="Y" />是&nbsp;&nbsp;
    		    	  <input type="radio" name="isexamine<%=w%>" value="N" checked />否
    		      <%}%>
