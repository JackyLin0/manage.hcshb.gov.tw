...
<!-- 
Programming Name: 駕駛人員維護 - 修改儲存
Programming File: drive101_mdysave.jsp
...
<%
  ...
  */
  ...
  DriveData removeobj = new DriveData();
  boolean removertn = true;
  removeobj.setTablename(table);
  removeobj.setStatedate(mdate);
  removeobj.setPubunitdn(pubunitdn);      		//發布單位DN
  removeobj.setPubunitname(pubunitname);    	//發布單位名稱
  removeobj.setSubject(mdate+"-刪除駕駛人員休假值班"); //標題
  removeobj.setPostname(logincn);           	//最後更新者姓名
  removeobj.setUnitname(title);             	//單元名稱
  removeobj.setTablename(table);    			//table名稱
  removeobj.setWebip(hostIP);               	//登入者IP
  removeobj.setLanguage(language);          	//語系
  removeobj.setStatus("D");                 	//狀態
  removertn = removeobj.removeDriverState();
