...
<%
  ...
  <%}else{
	  ...
	  for ( int i=0; i<ary_delno.length; i++ ) {
		  VaccinationTimeData qfile = new VaccinationTimeData();
		  ArrayList<Object> qdels = qfile.findBydel("VaccinationTimeFile",ary_delno[i]);	  
		  int delrcount = qfile.getAllrecordCount();
		  if ( qdels != null ) {
			  for ( int d=0; d<delrcount; d++ ) {
				  VaccinationTimeData qdel = ( VaccinationTimeData )qdels.get( d );
	  ...
	  VaccinationTimeData obj = new VaccinationTimeData();
	  ...
	  //執行動作(修改資料)  
	  rtn = obj.remove("VaccinationTimeFile");
	...
	<form name="mform" action="vaccination101.jsp" method="post">
