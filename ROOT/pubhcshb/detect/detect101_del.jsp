...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  BulletinData qfile = new BulletinData();
	  ArrayList qdels = qfile.findBydel("DetectFile",ary_delno[i]);	  	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  BulletinData qdel = ( BulletinData )qdels.get( d );
  ...
  //執行動作(修改資料)  
  rtn = obj.remove("DetectFile",sysRoot); 
...
<form name="mform" action="detect101.jsp" method="post">
