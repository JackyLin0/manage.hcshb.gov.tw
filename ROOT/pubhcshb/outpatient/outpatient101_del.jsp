...
<%
  ...
  <%}else{
	  ...
	  for ( int i=0; i<ary_delno.length; i++ ) {
		  OutpatientTimeData qfile = new OutpatientTimeData();
		  ArrayList<Object> qdels = qfile.findBydel("OutpatientTimeFile",ary_delno[i]);	  
		  int delrcount = qfile.getAllrecordCount();
		  if ( qdels != null ) {
			  for ( int d=0; d<delrcount; d++ ) {
				  OutpatientTimeData qdel = ( OutpatientTimeData )qdels.get( d );
	  ...
	  //執行動作(修改資料)  
	  rtn = obj.remove("OutpatientTimeFile");
