...
<%
  ...
  boolean rtn = meetData.load(serno,table1);
  
  MeetingData query1 = new MeetingData();
  ArrayList meetings = query1.findByday(table1, place, meetData.getSchedulDate(),"");
  String seartTime ="";
  String endTime ="";
  String boundTime ="";
  String tempserno ="";
  if(query1.getAllrecordCount()>0){
   for(int i=0 ; i<query1.getAllrecordCount() ; i++){
     ...
     if(!serno.equals(meeting.getSerno()))
 	  boundTime = boundTime + seartTime+ "~" + endTime+ "";
     if(!serno.equals(meeting.getSerno()) && i!=query1.getAllrecordCount()-1)
     	boundTime = boundTime +"||";
   }
//    out.println(boundTime);
  }
%>  
...
<script>
  ...
  function save(times)
  {
     ...
     document.mform.action = "placevisit201_mdysave.jsp";
     document.mform.method="post";
     document.mform.submit();
  }  
  ...
  function del(serno)
  {
     ...
     if(x)
     {
        document.mform.action="placevisit201_del.jsp?type=1";
        document.mform.method="post";
        document.mform.submit();
     } 
  }
...
<%if(username.equals(meetData.getPostname())){%>
 <a class="md" href="javascript:save('<%=boundTime%>')">儲存</a>
 <a class="md" href="javascript:del(<%=meetData.getSerno()%>)">刪除</a>
<%} %> 	
...
<%
  ...
  <tr>
    ...
    <td colspan="3">
      <input name="subject" type="text" class="lInput01" size="30" value="<%=meetData.getSubject()%>"/>
    </td>
  </tr>
  <tr>
    ...
    boolean rtn1 = query.load(meetData.getPlaceSerno(), table);
    %>
    
    <input name="placename" type="text" class="textarea" value="<%=query.getPlacename()%>" readonly size="20"/>
    <input name="placeSerno" type="hidden" value="<%=meetData.getPlaceSerno()%>"/>
    </td>
  </tr>
  <tr>
    ...
    <td colspan="3">
      <jsp:include page="qunit.jsp">
          ...
          <jsp:param name="datavalue" value="<%=meetData.getPubunitdn()%>"/>
      </jsp:include>
    </td>
  </tr>
  <tr>
    ...
    <td colspan="3">
      <input name="liaisonPer" type="text" class="lInput01" value="<%=meetData.getLiaisonPer()%>" size="10"/>
    </td>
  </tr>
  <tr>
    <td class="T12b" align="center"><span class="T12red">※</span>分機號碼</td>
    <td colspan="3">
      <input name="liaisonTel" type="text" class="lInput01" size="12" value="<%=meetData.getLiaisonTel()%>"/>
    </td>
  </tr>
  <tr>
    <td class="T12b" align="center"><span class="T12red">※</span>預定人數</td>
    <td colspan="3">
      <input name="schedulnum" type="text" class="lInput01" size="5" value="<%=meetData.getLimitNum()%>"/>
    </td>
  </tr>
  <tr>
    <td class="T12b" align="center"><span class="T12red">※</span>預定日期</td>
    <td colspan="3">
      <input name="sdate" value="<%=Datestr.date_chinese(meetData.getSchedulDate(), "")%>" type="text" class="textarea" size="15" readonly/>
      <input name="scheduldate" type="hidden" value="<%=meetData.getSchedulDate()%>" />
    </td>
  </tr>
  <tr>
    ...
    <td colspan="3">
      <select name="stime" class="select">
         <option value="<%=meetData.getSchedulSTime()%>"><%=meetData.getSchedulSTime().substring(0,2)%>:<%=meetData.getSchedulSTime().substring(2,4)%></option>
      ...
      <select name="etime" class="select">
         <option value="<%=meetData.getSchedulETime()%>"><%=meetData.getSchedulETime().substring(0,2)%>:<%=meetData.getSchedulETime().substring(2,4)%></option>
         <option value="0000">00:00</option>
         <option value="0030">00:30</option>
         <option value="0100">01:00</option>
         <option value="0130">01:30</option>
  ...
  <tr>
    ...
    <td colspan="3">
      <input type="radio" name="activityflag" value="Y" <%=(meetData.getActivityFlag().equals("Y") ? "checked" : "")%> />是&nbsp;&nbsp;
     <input type="radio" name="activityflag" value="N" <%=(meetData.getActivityFlag().equals("N") ? "checked" : "")%> />否
    </td>
  </tr>
  ...
  <tr>
    <td class="T12b" align="center"><span class="T12red">※</span>預定時間</td>
    <td colspan="3">
     <%=meetData.getSchedulSTime().substring(0,2)%>:<%=meetData.getSchedulSTime().substring(2,4)%> (起) 至
     <%=meetData.getSchedulETime().substring(0,2)%>:<%=meetData.getSchedulETime().substring(2,4)%> (迄) 止
    </td>
  </tr>
  <%}%>
