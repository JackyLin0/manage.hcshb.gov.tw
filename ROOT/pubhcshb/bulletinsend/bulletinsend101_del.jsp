...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  BulletinSendData qfile = new BulletinSendData();
	  ArrayList qdels = qfile.findBydel("BulletinsendFile",ary_delno[i]);	  	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  BulletinSendData qdel = ( BulletinSendData )qdels.get( d );
  ...
  //執行動作(修改資料)  
  rtn = obj.remove("BulletinSendFile",sysRoot); 
