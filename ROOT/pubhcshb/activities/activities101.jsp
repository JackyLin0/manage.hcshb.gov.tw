...
<script>
  function add()
  {
    document.mform.action="activities101_add.jsp";
  ...
  function mdy(serno)
  {
     ...
     document.mform.action="activities101_mdy.jsp";
     document.mform.method="post";
     document.mform.submit();
  } 
  ...
  function del()
  {
     ...
     if(x)
     {
        document.mform.action="activities101_del.jsp?type=0";
        document.mform.method="post";
        document.mform.submit();
     } 
  } 
  ...
  */
  function file(serno,filetype,tablename,path,language,pagesize)
  {
     ...
     }else if ( filetype == 'pic1' ) {
        window.open('../piclibrary/piclibrary_list.jsp?serno='+serno+'&table='+tablename+'&path='+path+'&language='+language+'&pagesize='+pagesize,'down','width=900,height=450,scrollbars=yes,left=90,top=90');   
...
<%
  ...
  ActivitiesData query = new ActivitiesData();    
  ArrayList qlists = query.findByday(table2,keyword,qpunit,sdate1,sdate2,"ActivityPoster","");  
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
  <tr align="center">
    <th width="3%">&nbsp;</th>
    <th width="4%">&nbsp;</th>
    <th width="30%" align="left"><%=lang.getSubject()%></th>
    <th width="8%"><%=lang.getShow()%></th>
    <th width="14%" align="left"><%=lang.getPubunitname()%></th>
    ...
    <th width="10%"><%=lang.getPiclibrary()%></th>
  </tr>
  <%
    ...
    for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	ActivitiesData qlist = ( ActivitiesData )qlists.get( i ); %>
    	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	  ...
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','doc','ActivityFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/file.gif" alt="file" width="16" height="16" border="0" align="middle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic','ActivityFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/Cmd_Save.gif" alt="attachments" width="16" height="16" border="0" align="middle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','media','ActivityFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/media1.gif" alt="media" width="16" height="16" border="0" align="middle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic1','ActivityFile','/piclibrary','<%=language%>','<%=pagesize%>')"><img src="../../img/add.gif" alt="引用圖庫" width="16" height="16" border="0" align="middle" /></a></td>    	  
    	</tr> 
    <%}%>
