...
<%
  ...
  //尋找活動標題
  String msubject = "";
  ActivityColumnData qdata = new ActivityColumnData();
  boolean rtn = qdata.load(serno);
  if ( rtn )
...
<script>
  ...
  function save(frcount)
  {
     ...
     document.mform.action = "activities102_mdysave.jsp";
     document.mform.method = "post";
     document.mform.submit();
  }    
</script>
  ...
  <%
    if ( qfields != null ) {
    	for ( int f=0; f<frcount; f++ ) {
        	...
        	ActivityColumnData qrecord = new ActivityColumnData();
        	String mfield = "";
        	boolean rtn1 = qrecord.loadrecord(serno,qfield.getSerno());        	
        	if ( rtn1 )
        		mfield = qrecord.getFieldserno();
        	//姓名及生日不可修改
        	String mdisable = "";
        	if ( qfield.getSerno().equals("200812030001") || qfield.getSerno().equals("200812030002") || qfield.getSerno().equals("200812030003") )
        		mdisable = " checked disabled";
        	//本活動報名表欄位需打勾    
        	String selchecked = "";
        	
        	if ( qfield.getSerno().equals(mfield) )
        		selchecked = "checked";
        	
        	//此欄位是否為必填
        	String needchecked = "";
        	if ( qrecord.getIsneed() != null && !qrecord.getIsneed().equals("null") && !qrecord.getIsneed().equals("") ) {
        		if ( qrecord.getIsneed().equals("Y") )
        			needchecked = "checked";
        	}  %>
        	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
        	  ...
        	  <td align="center"><input type="checkbox" name="check" value="<%=qfield.getSerno()%>" <%=mdisable%> <%=selchecked%>></td>
        	  <td align="center"><input type="checkbox" name="isneed<%=qfield.getSerno()%>" value="<%=qfield.getSerno()%>" <%=mdisable%> <%=needchecked%>></td>
