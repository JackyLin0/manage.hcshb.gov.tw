...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  ActivitiesData qfile = new ActivitiesData();	  
	  ArrayList qdels = qfile.findBydel("ActivityFile",ary_delno[i]);	 	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  ActivitiesData qdel = ( ActivitiesData )qdels.get( d );
  ...
  //執行動作(修改資料)  
  rtnweb = obj.remove("ActivityFile",sysRoot);  
...
<form name="mform" action="activities101.jsp" method="post">
