...
<%
  ...
  <%}else{
	  ...
	  for ( int i=0; i<ary_delno.length; i++ ) {
		  StatisticalData qfile = new StatisticalData();
		  ArrayList<Object> qdels = qfile.findBydel("StatisticalFile",ary_delno[i]);	  
		  int delrcount = qfile.getAllrecordCount();
		  if ( qdels != null ) {
			  for ( int d=0; d<delrcount; d++ ) {
				  StatisticalData qdel = ( StatisticalData )qdels.get( d );
	  ...
	  StatisticalData obj = new StatisticalData();
	  ...
	  //執行動作(修改資料)  
	  rtn = obj.remove("StatisticalFile");
	...
	<form name="mform" action="statistical101.jsp" method="post">
