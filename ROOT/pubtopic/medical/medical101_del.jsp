...
<%
  ...
  <%}else{
	  ...
	  for ( int i=0; i<ary_delno.length; i++ ) {
		  MedicalData qfile = new MedicalData();
		  ArrayList<Object> qdels = qfile.findBydel("MedicalFile",ary_delno[i]);	  
		  int delrcount = qfile.getAllrecordCount();
		  if ( qdels != null ) {
			  for ( int d=0; d<delrcount; d++ ) {
				  MedicalData qdel = ( MedicalData )qdels.get( d );
