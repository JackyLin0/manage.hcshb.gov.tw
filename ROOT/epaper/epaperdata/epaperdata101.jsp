...
<script>
  function add()
  {
    document.mform.action="epaperdata101_add.jsp";
  ...
  function mdy(serno)
  {
     ...
     document.mform.action="epaperdata101_mdy.jsp";
     document.mform.method="post";
     document.mform.submit();
  } 
  ...
  function del()
  {
     ...
     if(x)
     {
        document.mform.action="epaperdata101_del.jsp?type=0";
        document.mform.method="post";
        document.mform.submit();
     } 
  } 
...
<%
  ...
  ArrayList qlists = query.findByday(table,keyword,qpunit,qclass,flag);  
...
<div>
  ...
  <span class="T11b">狀態</span>
  <select name="flag">
     <option value="" <%=(flag.equals("") ? "selected" : "")%>>全部</option>
     <option value="Y" <%=(flag.equals("Y") ? "selected" : "")%>>已使用</option>
     <option value="N" <%=(flag.equals("N") ? "selected" : "")%>>未使用</option>
  </select>
...
<a class="md" href="javascript:quote()">引   用  話  說  健  康</a>
<%
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
  <tr align="center">
    <th width="5%">&nbsp;</th>
    <th width="5%">&nbsp;</th>
    <th width="33%" align="left">標題</th>
    <th width="15%" align="center">類別</th>
    <th width="10%">顯示</th>
    <th width="12%">更新日期</th>
    <th width="10%">文件檔案</th>
    <th width="10%">圖片檔案</th>
  ...
  <%
    ...
    for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	...
    	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	  ...
    	  <td align="center"><%=qlist.getMclassname()%></td>
    	  <td align="left">
    	  ...
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','doc','EpaperFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/file.gif" alt="file" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic','EpaperFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/Cmd_Save.gif" alt="attachments" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <!-- <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','media','EpaperFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/media1.gif" alt="media" width="16" height="16" border="0" align="absmiddle" /></a></td> -->
    	</tr> 
