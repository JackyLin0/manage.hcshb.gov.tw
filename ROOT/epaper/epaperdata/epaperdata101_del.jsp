...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  EpaperData qfile = new EpaperData();
	  ArrayList<Object> qdels = qfile.findBydel("EpaperFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  EpaperData qdel = ( EpaperData )qdels.get( d );
