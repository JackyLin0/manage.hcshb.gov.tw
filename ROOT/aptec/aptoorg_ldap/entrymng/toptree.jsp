	  ...
	  while ( qou.next())
	   {
	     ...
	     if ( (obj[0].equals("department")) || (obj[1].equals("department")) ) { %>
	    	 <span class="w2"><img src='../../image/org.gif'>&nbsp;<a style=text-Decoration:none href='../node.jsp?orgdn=<%=mdn%>' target='center'><%=qou.getString( "chinesetitle" )%></a></span><!,>toptree.jsp?sdn=<%=mdn%><!End>
  	     <%}else if ( (obj[0].equals("users")) || (obj[1].equals("users")) ) { %>
	         <span class="w2"><img src='../../image/user.gif'>&nbsp;<a style=text-Decoration:none href='../node.jsp?orgdn=<%=mdn%>' target='center'><%=qou.getString( "cn" )%></a></span><!,>toptree.jsp?sdn=<%=mdn%><!End>
	     <%}     
	   }     
	}%>
<!EndTreeData>
