...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  RestaurantData qfile = new RestaurantData();
	  ArrayList qdels = qfile.findBydel("RestaurantFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  RestaurantData qdel = ( RestaurantData )qdels.get( d );
  ...
  RestaurantData obj = new RestaurantData();
...
<form name="mform" action="restaurant101.jsp" method="post">
