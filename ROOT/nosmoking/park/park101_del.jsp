...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  ParkData qfile = new ParkData();
	  ArrayList qdels = qfile.findBydel("ParkFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  ParkData qdel = ( ParkData )qdels.get( d );
...
<form name="mform" action="park101.jsp" method="post">
