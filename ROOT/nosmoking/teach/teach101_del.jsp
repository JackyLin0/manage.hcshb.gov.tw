...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  TeachData qfile = new TeachData();
	  ArrayList<Object> qdels = qfile.findBydel("TeachFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  TeachData qdel = ( TeachData )qdels.get( d );
...
<form name="mform" action="teach101.jsp" method="post">
