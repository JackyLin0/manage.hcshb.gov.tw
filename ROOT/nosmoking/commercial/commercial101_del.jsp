...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  CommercialData qfile = new CommercialData();
	  ArrayList qdels = qfile.findBydel("CommercialFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  CommercialData qdel = ( CommercialData )qdels.get( d );
