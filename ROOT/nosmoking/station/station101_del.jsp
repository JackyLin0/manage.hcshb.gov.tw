...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  StationData qfile = new StationData();
	  ArrayList qdels = qfile.findBydel("StationFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  StationData qdel = ( StationData )qdels.get( d );
...
<form name="mform" action="station101.jsp" method="post">
