...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  OutpatientData qfile = new OutpatientData();
	  ArrayList qdels = qfile.findBydel("OutpatientFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  OutpatientData qdel = ( OutpatientData )qdels.get( d );
  ...
  OutpatientData obj = new OutpatientData();
