...
<script>    
  ...
  function add()
  {
    document.mform.action="register101_add.jsp";
  ...
  function mdy(serno)
  {
     ...
     document.mform.action="register101_mdy.jsp";
     document.mform.method="post";
     document.mform.submit();
  } 
  ...
  function del()
  {
     ...
     if(x)
     {
        document.mform.action="register101_del.jsp?type=0";
        document.mform.method="post";
        document.mform.submit();
     } 
  } 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table01">
  <tr align="center">
    <th width="5%">&nbsp;</th>
    <th width="5%">&nbsp;</th>
    <th width="35%" align="left">申辦項目名稱</th>
    <th width="10%">顯示</th>
  ...
  <%
    ...
    for ( int i = (startpage-1); ( ( i < endpage ) && ( i < rcount ) ); i ++ )  {
    	HouseRegisterData qlist = ( HouseRegisterData )qlists.get( i ); %>
    	<tr onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">
    	  ...
    	  <td align="left"><a href="javascript:mdy('<%=qlist.getSerno()%>')"><%=qlist.getItemname()%></a></td>
    	  <td align="left">
    	    <%
    	      String mstartusing = qlist.getStartusing();
    	      if ( mstartusing == null || mstartusing.equals("null") )
    	    	  mstartusing = "0";
    	    if ( mstartusing.equals("1") )  {  %>
    	    	<div align="center"><img src="../../img/publish_g.png" alt="發佈中" width="12" height="12" /></div>
    	    <%}else if ( mstartusing.equals("0") ) {%>
    	  ...
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','doc','HouseRegisterFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/file.gif" alt="file" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','pic','HouseRegisterFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/Cmd_Save.gif" alt="attachments" width="16" height="16" border="0" align="absmiddle" /></a></td>
    	  <!-- <td align="center"><a href="javascript:file('<%=qlist.getSerno()%>','media','HouseRegisterFile','<%=path%>','<%=language%>','<%=pagesize%>')"><img src="../../img/media1.gif" alt="media" width="16" height="16" border="0" align="absmiddle" /></a></td> -->
    	</tr> 
