...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  HouseRegisterData qfile = new HouseRegisterData();
	  ArrayList qdels = qfile.findByday("HouseRegisterFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  HouseRegisterData qdel = ( HouseRegisterData )qdels.get( d );
  ...
  //執行動作(修改資料)  
  rtn = obj.remove("HouseRegisterFile");  
