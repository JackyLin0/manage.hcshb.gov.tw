...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  HealthData qfile = new HealthData();
	  ArrayList qdels = qfile.findBydel("HealthFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  HealthData qdel = ( HealthData )qdels.get( d );
...
<form name="mform" action="health101.jsp" method="post">
