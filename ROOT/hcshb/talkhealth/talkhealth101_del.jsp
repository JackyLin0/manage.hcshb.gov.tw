...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  TalkHealthData qfile = new TalkHealthData();
	  ArrayList qdels = qfile.findBydel("TalkHealthFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  TalkHealthData qdel = ( TalkHealthData )qdels.get( d );
  ...
  TalkHealthData obj = new TalkHealthData();
...
<form name="mform" action="talkhealth101.jsp" method="post">
