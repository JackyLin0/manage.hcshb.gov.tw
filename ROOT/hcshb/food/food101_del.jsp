...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  FoodData qfile = new FoodData();
	  ArrayList qdels = qfile.findBydel("FoodFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  FoodData qdel = ( FoodData )qdels.get( d );
...
<form name="mform" action="food101.jsp" method="post">
