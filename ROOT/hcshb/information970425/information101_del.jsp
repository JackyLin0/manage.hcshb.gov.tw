...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  InformationData970425 qfile = new InformationData970425();
	  ArrayList qdels = qfile.findBydel("InformationFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  InformationData970425 qdel = ( InformationData970425 )qdels.get( d );
  ...
  //執行動作(修改資料)  
  rtn = obj.remove("InformationFile");
