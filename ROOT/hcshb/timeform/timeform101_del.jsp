...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  TimeFormData qfile = new TimeFormData();
	  ArrayList<Object> qdels = qfile.findBydel("TimeFormFile",ary_delno[i]);	  
	  int delrcount = qfile.getAllrecordCount();
	  if ( qdels != null ) {
		  for ( int d=0; d<delrcount; d++ ) {
			  TimeFormData qdel = ( TimeFormData )qdels.get( d );
  ...
  TimeFormData obj = new TimeFormData();
...
<form name="mform" action="timeform101.jsp" method="post">
