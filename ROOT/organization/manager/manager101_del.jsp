...
<%
  ...
  for ( int i=0; i<ary_delno.length; i++ ) {
	  ManagerData qfile = new ManagerData();	  
	  boolean rtn = qfile.loadfile(ary_delno[i]);
	  
	  if ( rtn ) {
		  uploadpath = upload.getProperty("picpath");
		  String filepath = uploadpath + path;
		  if ( qfile.getServerfile1() != null && !qfile.getServerfile1().equals("null") ) {
			  File df = new File(filepath,qfile.getServerfile1());
			  if (df.exists())
				  df.delete();
		  }
	  }
  ...
  obj.setTablename("OrganizeChief");   //table名稱
