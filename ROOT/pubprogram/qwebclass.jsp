...
<%
  ...
  TopicClassData qclassname = new TopicClassData();
  ArrayList<Object> qlists = qclassname.findByday(tablename,websitedn);  
  int rcount = qclassname.getAllrecordCount();
  
  //out.println("[程式名稱]: "+"qpubclass.jsp");
  //out.println("[tablename]: "+tablename);
  //out.println("[websitedn]: "+websitedn);
  //out.println("[資料筆數]: "+rcount);
  //out.println("ary_qclass: "+ary_qclass);
%>
  ...
  <%
    ...
    {    	    	 
       for ( int k=0; k<rcount; k++ ) {
    	   TopicClassData qlist = ( TopicClassData )qlists.get( k );
