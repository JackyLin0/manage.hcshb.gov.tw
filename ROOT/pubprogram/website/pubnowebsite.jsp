...
<!--
程式名稱：pubnowebsite.jsp
說明：資料多方發布站台(新增、修改-本次未建站台使用)
開發者：chmei
開發日期：97.02.16
修改者：
修改日期：
...
<%  
  ...
  String mlanguage = ( String )request.getParameter( "mlanguage" );
    
  //尋找所有站台
  WebSiteData qpubwebs = new WebSiteData();
  ArrayList qwebs = qpubwebs.findByday(mlanguage);
  int websrcount = qpubwebs.getAllrecordCount();
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <%
    if ( qpubwebs != null ) {
  	  for ( int w=0; w<websrcount; w++ ) {
  		  WebSiteData qweb = ( WebSiteData )qwebs.get( w );
   		  String webvalue = qweb.getWebsitedn() + "||" + qweb.getWebsitename();
   		  
   		  //尋找是否有發布至新竹縣入口網
   		  String mwebsitedn = exchange.getProperty(qweb.getWebsitedn());
   		  if ( mwebsitedn == null || mwebsitedn.equals("null") )
   			  mwebsitedn = qweb.getWebsitedn();
   		  ...
   		  <tr height="40%" onMouseOver="this.style.backgroundColor='#f4f4f4';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#ffffff';">    		    
   		    <%
   		      if ( qcnames != null ) { 
   		    	  String webisSelected = "";
   		    	  if ( SvString.contain(ary_mwebsitedn,qweb.getWebsitedn()) )
   		    		webisSelected = "checked";  %>   		    	  
   		    	  <td width="40%" align="left"><input type="checkbox" name=websitedn<%=w%> value=<%=webvalue%> <%=webisSelected%>><%=qweb.getWebsitename()%></td>
   		    	  <td align="left">
