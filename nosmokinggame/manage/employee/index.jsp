...
<%
	...
	sql = "select employee.* , limit_group.caption as limit_group" ;
	sql += " from employee" ;
	sql += " left join limit_group on employee.limit_group_sn = limit_group.sn" ;
	sql += " where " ;
	sql += " employee.uid like '%"+keyword+"%'" ;
	sql += " or employee.full_name like '%"+keyword+"%'" ;
	sql += " order by employee.uid" ;
	...
	if( f.cstr(request.getParameter("mode")).equals("delete") ) // 刪除
	{
		if( f.limit_function( function_sn , "delete" ) )
		{
			...
			f.execute_sql( "delete from employee where sn in("+sn_list+")" ) ;
...
<% for( i = 0 ; i < page_size ; i ++ ) { if( ! rs.next() ) break ; %>
	...
	<tr onClick="to_detail(<%=rs.getInt("sn")%>)" onMouseOver="this.style.backgroundColor='#CCFFCC'" onMouseOut="this.style.backgroundColor='<%=color%>'" style="cursor: hand ; background-Color:'<%=color%>'">
		...
		<td><%=rs.getString("limit_group")%></td>
