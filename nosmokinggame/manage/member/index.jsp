...
<%
	...
	String order = f.cstr(request.getParameter("order")) ;
	...
	sql += " order by issue_score1 desc" ;
	//out.print(sql);
	if( f.cstr(request.getParameter("mode")).equals("delete") ) // 刪除
	{
		if( f.limit_function( function_sn , "delete" ) )
		{
			...
			f.execute_sql( "delete from member where sn in("+sn_list+")" ) ;
...
<form method="post" action="index.jsp" name="form1">
	...
	<input type="hidden" name="order" value="total">
...
<table tabindex="9" cellspacing="1" cellpadding="0" bordercolor="Black" border="0" id="DataGrid1" style="color:Black;border-color:Black;border-width:0px;border-style:Solid;font-family:新細明體;font-size:X-Small;height:10px;width:100%;">
	<tr style="color:White;background-color:Brown;">
		...
		<td>姓名</td>
		<td>帳號</td>
		<!--td ><a href="javascript:to_order('issue_score1');"><%=f.get_code("issue","01")%></a></td>
		<td ><a href="javascript:to_order('issue_score2');"><%=f.get_code("issue","02")%></a></td>
		<td ><a href="javascript:to_order('issue_score3');"><%=f.get_code("issue","03")%></a></td>
		<td ><a href="javascript:to_order('issue_score4');"><%=f.get_code("issue","04")%></a></td-->
		<td >分數</td>
...
<% for( i = 0 ; i < page_size ; i ++ ) { if( ! rs.next() ) break ; %>
	...
	<tr onClick="to_detail(<%=rs.getInt("sn")%>)" onMouseOver="this.style.backgroundColor='#CCFFCC'" onMouseOut="this.style.backgroundColor='<%=color%>'" style="cursor: hand ; background-Color:'<%=color%>'">
		...
		<td><%
				ResultSet rs2 = f.query("select * from school where sn = "+ f.cstr(rs.getInt("school_sn")) )  ;
				rs2.next();
			%><%=rs2.getString("caption")%>
		</td>
		<td><%=f.get_code("year",rs.getString("year_code"))%></td>
		<td><%=rs.getString("name")%></td>
		<td><%=rs.getString("id")%></td>
		<!--td><%=rs.getInt("issue_score1")%></td>
		<td><%=rs.getInt("issue_score2")%></td>
		<td><%=rs.getInt("issue_score3")%></td>
		<td><%=rs.getInt("issue_score4")%></td-->
		<td><%=rs.getInt("issue_score1")%></td>
	</tr>
<% } %>
</table>
