...
<%
	...
	if( f.cint(request.getParameter("sn")) == 0 && allow_insert )
	{
		...
		rs = f.query("select * from game where sn = 0") ;
		rs.moveToInsertRow() ;
		sn = f.get_sn("game") ;
		rs.updateInt("sn",sn) ;
	...
	session.setAttribute("sn2" , sn) ;
	
	rs = f.query("select * from game where sn = "+String.valueOf(sn)) ;
...
<form name="form1" method="post" action="game_detail.jsp">
...
<table width="100%" cellspacing="0" cellpadding="2" border="1" align="center">
	...
	<tr>
		<td width="70"><font color="red">*</font>名稱：</td>
		<td><input type="text" name="caption" size="30" value="<%=f.field(rs,"caption")%>"></td>
	</tr>
	<tr>
		<td width="100" rowspsn="2">相關檔案：</td>
		<td>
			<iframe src="game_detail_sub.jsp?sn="<%=sn%>" width="600" height="70"></iframe>
		</td>
	</tr>
</form>
</table>
