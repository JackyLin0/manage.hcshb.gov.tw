...
<%
	...
	sql += " and practice_sn = " + request.getParameter("sn") ;
	sql += " order by sn" ;
	...
	if( f.cstr(request.getParameter("mode")).equals("delete") ) // 刪除
	{
		if( f.limit_function( function_sn , "delete" ) )
		{
			...
			f.execute_sql( "delete from game where sn in("+sn_list+")" ) ;
...
<script language="javascript">
	...
	{
		window.form1.action = "game_detail.jsp" ;
...
<form method="post" action="game_index.jsp" name="form1">
	<input type="hidden" name="mode" value="">
	<input type="hidden" name="sn" value="<%=request.getParameter("sn")%>">
	<input type="hidden" name="practice_sn" value="<%=request.getParameter("sn")%>">
