...
<%
	ResultSet rs = null ;
	Integer i = 0 ;
	rs = f.query("select * from game where sn = "+ session.getAttribute("sn2") ) ;
	...
	try{     
		...
		if( ! myFile.isMissing() )
		{
			myFile.saveAs(configure.files_path+"game\\"+myFile.getFileName());
...
<FORM name="form1" METHOD="POST" ACTION="game_detail_sub.jsp" ENCTYPE="multipart/form-data">
...
<table tabindex="9" cellspacing="1" cellpadding="0" bordercolor="Black" border="0" id="DataGrid1" style="color:Black;border-color:Black;border-width:0px;border-style:Solid;font-family:新細明體;font-size:X-Small;height:10px;width:100%;">
	...
	<tr>		
		<td>
			...
			<a href="<%=configure.root_path%>/files/game/<%=f.field(rs,"pic")%>" target="_blank"><%=f.cstr(f.field(rs,"pic"))%></a>
			<INPUT TYPE="hidden" NAME="FILE1_str" value="<%=f.cstr(f.field(rs,"pic"))%>">
		</td>
		<td>
			<INPUT TYPE="FILE" NAME="FILE2" SIZE="20">
			<a href="<%=configure.root_path%>/files/game/<%=f.field(rs,"file_1")%>" target="_blank"><%=f.cstr(f.field(rs,"file_1"))%></a>
