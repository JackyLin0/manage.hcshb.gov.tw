...
<%
	...
	ResultSet rs = f.query("select * from challenge_q where sn = "+String.valueOf(sn)) ;
...
<table width="100%" cellspacing="0" cellpadding="2" border="1" align="center">
	...
	<!--tr>
		<td width="70">問題類型：</td>
		<td><%=f.select_code("issue_code","issue",f.field(rs,"issue_code"), false ,"")%></td>
	</tr-->
	<tr>
		<td>題目：</td>
		<td><textarea name="caption" cols=80 rows=10><%=f.field(rs,"caption").replaceAll("<", "&lt;") %></textarea></td>
	</tr>
	<tr>
		<td>題目圖片：</td>
		<td><iframe src="detail_sub2.jsp?function_sn=<%=function_sn%>&sn=<%=sn%>&time=<%=f.urlencode(f.now())%>" width="600" height="50"></iframe></td>
	</tr>
	<tr>
		<td>答案：</td>
		<td><iframe src="detail_sub.jsp?function_sn=<%=function_sn%>&challenge_q_sn=<%=sn%>&time=<%=f.urlencode(f.now())%>" width="600" height="300"></iframe></td>
	</tr>
