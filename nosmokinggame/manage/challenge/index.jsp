...
<%
	...
	sql = "select *" ;
	sql += " from challenge_q" ;
	sql += " where caption      like '%"+keyword+"%'" ;
	sql += " order by issue_code" ;
	...
	if( f.cstr(request.getParameter("mode")).equals("delete") ) // 刪除
	{
		if( f.limit_function( function_sn , "delete" ) )
		{
			...
			f.execute_sql( "delete from challenge_q where sn in("+sn_list+")" ) ;
			f.execute_sql( "delete from challenge_a where challenge_q_sn in("+sn_list+")" ) ;
...
<% for( i = 0 ; i < page_size ; i ++ ) { if( ! rs.next() ) break ; %>
	...
	<tr onClick="to_detail(<%=rs.getInt("sn")%>)" onMouseOver="this.style.backgroundColor='#CCFFCC'" onMouseOut="this.style.backgroundColor='<%=color%>'" style="cursor: hand ; background-Color:'<%=color%>'">
		...
		<td>
		<%
			if(!f.cstr(rs.getString("pic")).equals(""))
			{
				if( rs.getString("place_code").equals("01"))
				{
					out.print("<img src='"+configure.root_path + "/files/challenge/" + rs.getString("pic") + "'/>" + rs.getString("caption"));
				}else{
					out.print(rs.getString("caption") + "<img src='"+configure.root_path + "/files/challenge/" + rs.getString("pic") + "'/>");
				}
			}else{
