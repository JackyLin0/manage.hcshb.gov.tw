...
<%
	...
	sql = "select *" ;
	sql += " from employee_log" ;
	sql += " where 1=1" ;
	sql += " and ( full_name like '%"+keyword+"%'" ;
	sql += "    or ip like '%"+keyword+"%'" ;
	sql += "    or caption like '%"+keyword+"%'" ;
	sql += "    or remark like '%"+keyword+"%' )" ;
	if( ! f.cstr(request.getParameter("d1")).equals("") ) sql += " and log_time >= '"+request.getParameter("d1")+" 00:00:00'" ;
	if( ! f.cstr(request.getParameter("d2")).equals("") ) sql += " and log_time <= '"+request.getParameter("d2")+" 23:59:59'" ;
	...
	if( f.cstr(request.getParameter("mode")).equals("delete") ) // 刪除
	{
		if( f.limit_function( function_sn , "delete" ) )
		{
			...
			f.execute_sql( "delete from employee_log where sn in("+sn_list+")" ) ;
...
日期: <%=f.date_picker("d1",f.cstr(request.getParameter("d1")))%> 至 <%=f.date_picker("d2",f.cstr(request.getParameter("d2")))%> 關鍵字:
