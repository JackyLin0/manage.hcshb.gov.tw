<?php
/*
	CVS Revision. 1.1.0 modify from Ianweng at 2002/2/1 
*/
...
//	************************************************************
	...
	function FastSave ( $template = "",$save_path = "./a.html" )
	{
		if(empty($template))
		{
			$template = $this->LAST;
		...
		if( (!(isset($this->$template))) || (empty($this->$template)) )
		{
			$this->error("Nothing parsed, nothing saved",0);
			return;
		}
		else
		{
		      if( $yes_no=fopen("$save_path","w"))
                         fputs($yes_no,$this->$template);
                      else
                      {
                       	 $this->error("Cannot save,Please check filename or directory",0);
			 return; 
                      }   
		}
		return;
...
//	************************************************************
	...
	function assignMoney ($tpl_array, $trailer="")
	{
		if(gettype($tpl_array) == "array")
		{
			while ( list ($key,$val) = each ($tpl_array) )
			{
				if (!(empty($key)))
				{
					//	Empty values are allowed
					//	Empty Keys are NOT
					$this->PARSEVARS["$key"] = number_format($val);
					if ($this->PARSEVARS["$key"]=='0' and $val!='0')
					{
						$this->PARSEVARS["$key"] = '--';
					}
				}
		...
		else
		{
			...
			if (!empty($tpl_array))
			{
				$this->PARSEVARS["$tpl_array"] = number_format($trailer);
				if ($this->PARSEVARS["$tpl_array"]=='0' and $trailer!='0')
				{
...
//	************************************************************
	...
	function assignMoney2 ($tpl_array, $trailer="")
	{
		if(gettype($tpl_array) == "array")
		{
			while ( list ($key,$val) = each ($tpl_array) )
			{
				if (!(empty($key)))
				{
					//	Empty values are allowed
					//	Empty Keys are NOT
					$this->PARSEVARS["$key"] = number_format($val,2,'.',',');
					if ($this->PARSEVARS["$key"]=='0' and $val!='0')
					{
						$this->PARSEVARS["$key"] = '--';
					}
		...
		else
		{
			...
			if (!empty($tpl_array))
			{
				$this->PARSEVARS["$tpl_array"] = number_format($trailer,2,'.',',');
				if ($this->PARSEVARS["$tpl_array"]=='0' and $trailer!='0')
				{
					$this->PARSEVARS["$tpl_array"] = '--';
				}
